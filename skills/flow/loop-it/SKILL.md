...
- 批末把各 issue 分支汇总到一条批次分支（`git merge --no-ff` 各分支，或直接在累积分支上顺序 commit；**`failed` 的分支不要并入**），`/review-it` 看这条分支相对 `main` 的 diff，`/ship-it` 从它开一个 PR。批末 PR 关闭多个 issue，因此按 `/ship-it` 的「多个 issue 共用一个 PR」逐项列出 commit / 关闭的 issue / 验收证据 / 人工验收状态——否则单个 issue 的实现无法追溯与回滚。
...
| 在默认分支 | `git branch --show-current` | 提示 `git checkout main && git pull` 切回 |
...
git checkout main && git pull && git checkout -b feat/issue-N-slug
```
...
- 每个 issue 结束后切回 `main`；**失败的分支保留**，不要删。
...
- 每次 bash 调用都是全新 shell：`cd`、变量不跨调用保留；`git checkout main && git pull` 这类多步必须写在同一条命令里。
