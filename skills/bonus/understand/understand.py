#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""understand.py — 把一段 git diff 变成浅色双栏的 code-review 网页。
...
流程：
  ...
  2. 调用方 agent 编辑 <out>/annotations.json，为关键代码段补「相关单位需求」与「代码解释」。
...
def cmd_scan(args):
    ...
    # annotations 骨架：仅在不存在时创建，避免覆盖已写入的注释
...
def main():
    ap = argparse.ArgumentParser(description="浅色双栏 code-review 网页生成器")
