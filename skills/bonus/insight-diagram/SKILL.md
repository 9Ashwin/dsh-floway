...
description: 为任意项目生成 UML 图、架构图和流程图。分析代码库后让用户选择要生成的图表类型，以本技能自带的示例模板渲染 HTML+SVG（若环境中装有 architecture-diagram skill 也可调用它），保存到 docs/ 目录。适用于任何软件项目的文档可视化。
...
分析任意代码库，自动生成 UML 14种图 + 架构图 + 流程图，渲染为 HTML+SVG。渲染方式见「步骤 3」：优先复用本技能 `examples/` 里的模板自行生成；只有在环境中确实装有 `architecture-diagram` skill 时才调用它。
